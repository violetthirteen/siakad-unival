...
@section('title', 'Tambah Semester - SIAKAD Unival')

@section('content')
<div class="page-header">
    <div><h1>Tambah Semester</h1><p>Buat semester baru</p></div>
    <a href="{{ route('admin.semester.index') }}" class="btn-modern btn-ghost"><i class="bi bi-arrow-left"></i> Kembali</a>
</div>
<div class="card-modern">
    <div class="card-modern-body">
        <form method="POST" action="{{ route('admin.semester.store') }}" class="form-modern">
            @csrf
            <div class="row">
                <div class="col-md-6">
                    ...
                    <select name="tahun_ajaran_id" class="form-control @error('tahun_ajaran_id') is-invalid @enderror" required>
                        ...
                        @foreach($tahunAjaran as $ta)
                            <option value="{{ $ta->id }}" {{ old('tahun_ajaran_id') == $ta->id ? 'selected' : '' }}>{{ $ta->tahun_awal }}/{{ $ta->tahun_akhir }}</option>
                        @endforeach
                    </select>
