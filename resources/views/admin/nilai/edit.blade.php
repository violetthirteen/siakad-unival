...
@section('title', 'Edit Nilai - SIAKAD Unival')

@section('content')
<div class="page-header">
    <div><h1>Edit Nilai</h1><p>Ubah data nilai</p></div>
    <a href="{{ route('admin.nilai.index') }}" class="btn-modern btn-ghost"><i class="bi bi-arrow-left"></i> Kembali</a>
</div>
<div class="card-modern">
    <div class="card-modern-body">
        <form method="POST" action="{{ route('admin.nilai.update', $nilai) }}" class="form-modern">
            @csrf @method('PUT')
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <select name="mahasiswa_id" class="form-control @error('mahasiswa_id') is-invalid @enderror" required>
                        ...
                        @foreach($mahasiswa as $m)
                            <option value="{{ $m->id }}" {{ old('mahasiswa_id', $nilai->mahasiswa_id) == $m->id ? 'selected' : '' }}>{{ $m->nim }} - {{ $m->nama_lengkap }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select name="mata_kuliah_id" class="form-control @error('mata_kuliah_id') is-invalid @enderror" required>
                        ...
                        @foreach($mataKuliah as $mk)
                            <option value="{{ $mk->id }}" {{ old('mata_kuliah_id', $nilai->mata_kuliah_id) == $mk->id ? 'selected' : '' }}>{{ $mk->kode_mk }} - {{ $mk->nama_mk }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select name="semester_id" class="form-control @error('semester_id') is-invalid @enderror" required>
                        ...
                        @foreach($semester as $s)
                            <option value="{{ $s->id }}" {{ old('semester_id', $nilai->semester_id) == $s->id ? 'selected' : '' }}>{{ $s->nama_semester }} {{ $s->tahunAjaran->tahun_awal ?? '' }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-2 mb-3">
                    ...
                    <input type="number" name="nilai_tugas" class="form-control @error('nilai_tugas') is-invalid @enderror" value="{{ old('nilai_tugas', $nilai->nilai_tugas) }}" step="0.01">
                    @error('nilai_tugas') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-2 mb-3">
                    <label class="form-label">Nilai UTS</label>
                    <input type="number" name="nilai_uts" class="form-control @error('nilai_uts') is-invalid @enderror" value="{{ old('nilai_uts', $nilai->nilai_uts) }}" step="0.01">
                    @error('nilai_uts') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-2 mb-3">
                    <label class="form-label">Nilai UAS</label>
                    <input type="number" name="nilai_uas" class="form-control @error('nilai_uas') is-invalid @enderror" value="{{ old('nilai_uas', $nilai->nilai_uas) }}" step="0.01">
