...
@section('title', 'Edit Dosen - SIAKAD Unival')

@section('content')
<div class="page-header">
    <div><h1>Edit Dosen</h1><p>Ubah data dosen</p></div>
    <a href="{{ route('admin.dosen.index') }}" class="btn-modern btn-ghost"><i class="bi bi-arrow-left"></i> Kembali</a>
</div>
<div class="card-modern">
    <div class="card-modern-body">
        <form method="POST" action="{{ route('admin.dosen.update', $dosen) }}" class="form-modern">
            @csrf @method('PUT')
            <div class="row">
                <div class="col-md-6">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="nama_lengkap" class="form-control @error('nama_lengkap') is-invalid @enderror" value="{{ old('nama_lengkap', $dosen->nama_lengkap) }}" required>
                    @error('nama_lengkap') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-4">
                    <label class="form-label">Tempat Lahir</label>
                    <input type="text" name="tempat_lahir" class="form-control @error('tempat_lahir') is-invalid @enderror" value="{{ old('tempat_lahir', $dosen->tempat_lahir) }}">
                    @error('tempat_lahir') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-4">
                    <label class="form-label">Tanggal Lahir</label>
                    <input type="date" name="tanggal_lahir" class="form-control @error('tanggal_lahir') is-invalid @enderror" value="{{ old('tanggal_lahir', $dosen->tanggal_lahir ? $dosen->tanggal_lahir->format('Y-m-d') : '') }}">
                ...
                <div class="col-md-6">
                    ...
                    <textarea name="alamat" class="form-control @error('alamat') is-invalid @enderror">{{ old('alamat', $dosen->alamat) }}</textarea>
                ...
                <div class="col-md-6">
                    ...
                    <select name="program_studi_id" class="form-control @error('program_studi_id') is-invalid @enderror" required>
                        ...
                        @foreach($programStudi as $p)
                            <option value="{{ $p->id }}" {{ old('program_studi_id', $dosen->program_studi_id) == $p->id ? 'selected' : '' }}>{{ $p->nama_prodi }}</option>
                        @endforeach
                    </select>
