...
@section('title', 'Edit Mata Kuliah - SIAKAD Unival')

@section('content')
<div class="page-header">
    <div><h1>Edit Mata Kuliah</h1><p>Ubah data mata kuliah</p></div>
    <a href="{{ route('admin.mata-kuliah.index') }}" class="btn-modern btn-ghost"><i class="bi bi-arrow-left"></i> Kembali</a>
</div>
<div class="card-modern">
    <div class="card-modern-body">
        <form method="POST" action="{{ route('admin.mata-kuliah.update', $mataKuliah) }}" class="form-modern">
            @csrf @method('PUT')
            <div class="row">
                <div class="col-md-4">
                ...
                <div class="col-md-2">
                    ...
                    <input type="number" name="sks" class="form-control @error('sks') is-invalid @enderror" value="{{ old('sks', $mataKuliah->sks) }}" required>
                    @error('sks') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-2">
                    <label class="form-label">Semester</label>
                    <input type="number" name="semester" class="form-control @error('semester') is-invalid @enderror" value="{{ old('semester', $mataKuliah->semester) }}" required>
                ...
                <div class="col-md-6">
                    ...
                    <select name="program_studi_id" class="form-control @error('program_studi_id') is-invalid @enderror" required>
                        ...
                        @foreach($programStudi as $p)
                            <option value="{{ $p->id }}" {{ old('program_studi_id', $mataKuliah->program_studi_id) == $p->id ? 'selected' : '' }}>{{ $p->nama_prodi }}</option>
                        @endforeach
                    </select>
