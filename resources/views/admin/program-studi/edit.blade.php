...
@section('title', 'Edit Program Studi - SIAKAD Unival')

@section('content')
<div class="page-header">
    <div><h1>Edit Program Studi</h1><p>Ubah data program studi</p></div>
    <a href="{{ route('admin.program-studi.index') }}" class="btn-modern btn-ghost"><i class="bi bi-arrow-left"></i> Kembali</a>
</div>
<div class="card-modern">
    <div class="card-modern-body">
        <form method="POST" action="{{ route('admin.program-studi.update', $programStudi) }}" class="form-modern">
            @csrf @method('PUT')
            <div class="row">
                <div class="col-md-4">
                    <label class="form-label">Kode Prodi</label>
                    <input type="text" name="kode_prodi" class="form-control @error('kode_prodi') is-invalid @enderror" value="{{ old('kode_prodi', $programStudi->kode_prodi) }}" required>
                ...
                <div class="col-md-6">
                    ...
                    <select name="fakultas_id" class="form-control @error('fakultas_id') is-invalid @enderror" required>
                        ...
                        @foreach($fakultas as $f)
                            <option value="{{ $f->id }}" {{ old('fakultas_id', $programStudi->fakultas_id) == $f->id ? 'selected' : '' }}>{{ $f->nama_fakultas }}</option>
                        @endforeach
                    </select>
